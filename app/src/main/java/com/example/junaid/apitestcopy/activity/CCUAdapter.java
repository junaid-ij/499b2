...
public class CCUAdapter extends RecyclerView.Adapter<CCUAdapter.ProductViewHolder> {

    private Context mCtx;
    private List<CCUStructure> CCUList;

    public CCUAdapter(Context mCtx, List<CCUStructure> CCUList) {
        this.mCtx = mCtx;
        this.CCUList = CCUList;
    }

    @Override
    public CCUAdapter.ProductViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        View view = inflater.inflate(R.layout.search_list, null);
        return new CCUAdapter.ProductViewHolder(view);
    }

    @Override
    public void onBindViewHolder(CCUAdapter.ProductViewHolder holder, int position) {
        final CCUStructure product = CCUList.get(position);
        ...
        holder.hos_numofbeds.setText(product.getNumofccu());
